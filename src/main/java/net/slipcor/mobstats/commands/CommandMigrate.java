...
public class CommandMigrate extends AbstractCommand {
    public CommandMigrate() {
        super(new String[]{"mobstats.migrate"});
    }

    @Override
    public void commit(CommandSender sender, String[] args) {
        if (!hasPerms(sender)) {
            MobStats.getInstance().sendPrefixed(sender, Language.MSG_NOPERMMIGRATE.toString());
            return;
        }
        ...
        if (args[2].toLowerCase().equals("mysql") ||
            ...
            MobStats.getInstance().sendPrefixed(sender, Language.ERROR_COMMAND_ARGUMENT.toString(args[2], "'mysql' or 'sqlite' or 'yml'"));
            return;
        }
        ...
        if (args[1].toLowerCase().equals("from")) {
            ...
            if (result >= 0) {
                if (result > 0) {
                    MobStats.getInstance().sendPrefixed(sender, Language.MSG_MIGRATED.toString(String.valueOf(result)));
                } else {
                    MobStats.getInstance().sendPrefixed(sender, Language.MSG_MIGRATE_EMPTY.toString());
                }
            }
        ...
        MobStats.getInstance().sendPrefixed(sender, Language.ERROR_COMMAND_ARGUMENT.toString(args[1], "'from' or 'to'"));
    }
