...
public class MetricsMain extends CoreMetrics {
    ...
    public MetricsMain(Plugin plugin) {
        ...
        this.addChart(Config.Entry.OTHER_MOBARENA, "use_mobarena");

        MobStats.getInstance().getLogger().info("sending full Metrics! You can deactivate this in the config.yml");
    }
    ...
    private void addConnectionChart(Config.Entry mysql, Config.Entry sqlite) {
        this.addCustomChart(new SimplePie("connection_type", new Callable<String>() {
            ...
            public String call() throws Exception {
                if (MobStats.getInstance().config().getBoolean(mysql)) {
                    return "mySQL";
                }
                if (MobStats.getInstance().config().getBoolean(sqlite)) {
                    return "SQLite";
                }
                return "YML";
            }
    ...
    private void addChart(Config.Entry cfg, String id) {
        this.addCustomChart(new SimplePie(id, new Callable<String>() {
            @Override
            public String call() throws Exception {
                return String.valueOf(MobStats.getInstance().config().getBoolean(cfg));
            }
        }));
    }
